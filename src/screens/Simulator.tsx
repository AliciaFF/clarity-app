    ...
    <div>
      ...
      {step === 'result' && account && detectedPrice !== null && (
        <div style={{ padding: '20px 20px 32px' }}>
          <div style={{ background: canAfford ? 'linear-gradient(135deg, #D4A840, #C9A040, #B8902E)' : '#FFEBEE', borderRadius: 14, padding: '14px 18px', textAlign: 'center', marginBottom: 24 }}>
            <p style={{ fontSize: 11, color: canAfford ? 'rgba(255,255,255,0.75)' : '#9AA5B4', marginBottom: 4, textTransform: 'uppercase', letterSpacing: 0.5, fontWeight: 600 }}>Solde après achat</p>
            <p style={{ fontSize: 28, fontWeight: 800, color: remaining! < 0 ? '#EF5350' : '#fff' }}>
              {fmt(remaining!)} €
            </p>
          </div>
          ...
          <p className="section-title" style={{ marginBottom: 14 }}>Projection après achat</p>
          <div style={{ display: 'flex', gap: 10, marginBottom: 36 }}>
            {projections.map(({ days, balance }) => (
              <div key={days} style={{ flex: 1, background: '#fff', borderRadius: 14, padding: '18px 8px', textAlign: 'center', boxShadow: '0 2px 6px rgba(0,0,0,0.06)' }}>
          ...
          <button className="btn-primary" style={{ marginTop: 14, background: '#0D0D0D', color: '#C9A040', border: 'none' }}
